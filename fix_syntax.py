...
# I will find the start of the broken string and replace the whole block again.
...
# I will properly double-escape the math backslashes so they are correctly printed to the TS file
import re
new_content_escaped = new_content.replace("\\", "\\\\")

# BUT wait! We MUST NOT double escape backticks if they are meant to be escaped backticks for the template literal!
# In the raw string above, I used \`\`\` for code blocks.
# If I do replace("\", "\\"), it becomes \\`\\`\\`
# Which means JS sees \`\`\`
# Actually, inside a template string `...`, the character ` must be escaped as \`.
# If I write \` on disk, JS evaluates it as a literal backtick. That's perfect.
# If I write \\` on disk, JS evaluates it as \ followed by a backtick. That closes the template literal!
# So I must FIX the backticks.
new_content_escaped = new_content_escaped.replace("\\\\`", "\\`")

text = text[:start_idx] + new_content_escaped + "\n" + end_marker + text[end_idx + len(end_marker):]
